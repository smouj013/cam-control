/* player.js — CamStudio Room PLAYER (v1.0.1)
   ✅ Compatible con player.html (IDs intactos)
   ✅ FIX crítico: handlePlayFail usaba "reason" fuera de scope
   ✅ FIX crítico: fallback YouTube iframe NO marcaba ON-AIR (ahora sí)
   ✅ Stop local corta rotación + timers (no vuelve a arrancar solo)
   ✅ Estado + ACK robustos (BroadcastChannel + localStorage fallback)
*/
(() => {
  "use strict";
  ...
  function clearStage() {
    try { PLAY.yt?.destroy?.(); } catch {}
    PLAY.yt = null;

    try { PLAY.hls?.destroy?.(); } catch {}
    PLAY.hls = null;

    if (UI.playerArea) UI.playerArea.innerHTML = "";
    PLAY.elem = null;
    PLAY.alive = false;
    setLive(false);
  }
  ...
  function normalizeCam(cam) {
    if (!cam) return null;
    return CATALOG.byId.get(cam.id) || cam || null;
  }
  ...
  function setMuted(m) {
    ...
    // - si está muted: acción -> Unmute
    // - si está con audio: acción -> Mute
    ...
    try {
      if (PLAY.yt?.mute && PLAY.yt?.unMute) {
  ...
  async function playCam(cam, { reason = "manual", noRotateReset = false } = {}) {
    ...
    if (cam.kind === "youtube") {
      ...
      if (!okApi) {
        ...
        iframe.src =
          `https://www.youtube-nocookie.com/embed/${encodeURIComponent(cam.src)}` +
          `?autoplay=1&mute=${PLAY.muted ? 1 : 0}&controls=0&modestbranding=1&rel=0&playsinline=1`;
        wrap.appendChild(iframe);
        PLAY.elem = iframe;

        const ok = await healthCheckIframe(iframe, token);
        if (!ok) return handlePlayFail(cam, token, "YouTube (iframe) no arrancó", reason);
      ...
      if (!ytReady) return handlePlayFail(cam, token, "YouTube no arrancó", reason);
      return handlePlayOk(cam, token, reason);
    }
    ...
    if (cam.kind === "hls") {
      ...
      if (canNative) {
        ...
        if (!ok) return handlePlayFail(cam, token, "HLS nativo falló", reason);
        return handlePlayOk(cam, token, reason);
      }
      ...
      if (!okHls) {
        v.src = cam.src;
        const ok = await healthCheckVideo(v, token);
        if (!ok) return handlePlayFail(cam, token, "HLS requiere hls.js", reason);
        return handlePlayOk(cam, token, reason);
      }
      ...
      try {
        ...
        return handlePlayFail(cam, token, "hls.js attach falló", reason);
      }

      const ok = await healthCheckVideo(v, token);
      if (!ok) return handlePlayFail(cam, token, "HLS no arrancó", reason);
      return handlePlayOk(cam, token, reason);
    }
    ...
    if (cam.kind === "image") {
      ...
      if (!ok) return handlePlayFail(cam, token, "Imagen no carga", reason);
      return handlePlayOk(cam, token, reason);
    }

    return handlePlayFail(cam, token, "Tipo no soportado", reason);
  }
  ...
  async function handlePlayFail(cam, token, why, reason) {
    if (token !== PLAY.playToken) return false;
